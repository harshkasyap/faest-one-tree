...
// returns 0 if all went well, negative numbers otherwise. 
template<typename T>
int sha3_384(uint8_t * output, const T * input, const size_t length = 1) {
	EVP_MD_CTX * mdctx;
	const EVP_MD * algo = EVP_sha3_384();
...
// returns 0 if all went well, negative numbers otherwise. 
template<typename T>
int sha3_512(uint8_t * output, const T * input, const size_t length = 1) {
	EVP_MD_CTX * mdctx;
