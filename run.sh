...
for i in $(seq 1 10); do
    ...
    sign_time=$(echo "$output" | grep "Time taken to sign:" | sed -E 's/.*Time taken to sign: ([0-9.]+) seconds.*/\1/')
    verify_time=$(echo "$output" | grep "Time taken to verify:" | sed -E 's/.*Time taken to verify: ([0-9.]+) seconds.*/\1/')
